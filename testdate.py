...
sdate = lambda x: datetime.datetime.strptime(x, '%m %Y')
# sdate = lambda x: datetime.datetime.strptime(x, '%m %Y %H:%M:%S.%f')

df_review['user_sentiment'] = np.select(conditions, classify)
df_review['date'] = df_review['date_of_stay'].apply(sdate)
...
#
df_review['polarity'] = df_review['comment'].apply(pol)
df_review['subjectivity'] = df_review['comment'].apply(sub)
